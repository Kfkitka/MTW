 ...
 */
abstract class SeamlessDonationsAdminPageFramework_Factory_Controller extends SeamlessDonationsAdminPageFramework_Factory_View {
    ...
    public function setFieldErrors($aErrors) {
        $GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'] = $this->oUtil->getElement($GLOBALS, array('aSeamlessDonationsAdminPageFramework', 'aFieldErrors'), array());
        if (empty($GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'])) {
            add_action('shutdown', array($this, '_replyToSaveFieldErrors'));
        }
        $_sID = md5($this->oProp->sClassName);
        $GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'][$_sID] = isset($GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'][$_sID]) ? $this->oUtil->uniteArrays($GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'][$_sID], $aErrors) : $aErrors;
    }
    public function hasFieldError() {
        return isset($GLOBALS['aSeamlessDonationsAdminPageFramework']['aFieldErrors'][md5($this->oProp->sClassName) ]);
    }
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = array(), $bOverride = true) {
        $GLOBALS['aSeamlessDonationsAdminPageFramework']['aNotices'] = $this->oUtil->getElement($GLOBALS, array('aSeamlessDonationsAdminPageFramework', 'aNotices'), array());
        if (empty($GLOBALS['aSeamlessDonationsAdminPageFramework']['aNotices'])) {
            add_action('shutdown', array($this, '_replyToSaveNotices'));
        }
        $_sID = md5(trim($sMessage));
        if ($bOverride || !isset($GLOBALS['aSeamlessDonationsAdminPageFramework']['aNotices'][$_sID])) {
            ...
            $GLOBALS['aSeamlessDonationsAdminPageFramework']['aNotices'][$_sID] = array('sMessage' => $sMessage, 'aAttributes' => $_aAttributes + array('class' => $sType, 'id' => $this->oProp->sClassName . '_' . $_sID,),);
        }
    }
    public function hasSettingNotice($sType = '') {
        $_aNotices = $this->oUtil->getElementAsArray($GLOBALS, array('aSeamlessDonationsAdminPageFramework', 'aNotices'), array());
        if (!$sType) {
